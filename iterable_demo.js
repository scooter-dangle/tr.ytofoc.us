...
(function() {
  ...
  $(document).ready(function() {
    var debug, debug_count, page, phone_home, reset_queue, route, routes, slide, update_queue, _ref;
    debug_count = 0;
    if (debugging) {
    ...
    _ref = (function() {
      var abs_height, abs_width, charts, colors, confine, debugging, demo_object, duration, max, methods, min, queue, reset_queue, standard_delay, update_data, update_queue;
      abs_width = 490;
      abs_height = 180;
      debugging = false;
      max = function(x, y) {
      ...
      confine = function(n, infimum, supremum) {
        return min(max(n, infimum), supremum);
      };
      ...
      update_data = function(parcel) {
        ...
        elems = d3.select('#assets defs').selectAll('.elem').data(catalogue, function(d) {
          return d;
        }).enter().append('svg').attr('id', function(d) {
          return "iterable_elem_" + d;
      ...
      demo_object = function(datar) {
        ...
        joiner.enter().append('svg').classed('mover', true).each(function(d) {
          return d3.select(this).classed("iterable_elem_" + d, true);
      ...
      methods = function(selection) {
        ...
        return yielder.enter().append('use').attr('xlink:href', function(d) {
          ...
          return $(this).parents('.chart').find(".iterable_elem_" + d).toggle('visible').toggle('visible');
        });
      };
      queue = [];
      ...
      update_queue = function(parcel) {
        if (queue[0] !== parcel) {
          return queue.unshift(parcel);
        }
      };
      setInterval((function() {
        if (queue.length) {
          return update_data(queue.pop());
        }
      }), duration * 2);
      return [update_queue, reset_queue];
    })(), update_queue = _ref[0], reset_queue = _ref[1];
    slide = function(num) {
      var num_slides;
    ...
    page = function(name) {
      ...
      return d3.selectAll("#" + name).attr('hidden', null);
    };
    routes = {
      slide: slide,
      page: page,
